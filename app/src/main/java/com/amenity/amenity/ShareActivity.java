...
public class ShareActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final ArrayList<Integer> have=new ArrayList<>(0);
        ...
        share.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(((chkwater.isChecked())&&(TextUtils.isEmpty(qtywater.getText().toString())))||((chkblanket.isChecked())&&(qtyblanket.getText().toString().matches("")))||((chkfood.isChecked())&&(qtyfood.getText().toString().matches("")))||((chkmedicines.isChecked())&&(qtymedicines.getText().toString().matches("")))||((chktoiletries.isChecked())&&(qtytoiletries.getText().toString().matches("")))) {
                    ...
                    if(chkwater.isChecked()){
                        resources.add("Water");
                        have.add(Integer.parseInt(qtywater.getText().toString()));
                    }

                    if(chkblanket.isChecked()) {
                        resources.add("Blankets");
                        have.add(Integer.parseInt(qtyblanket.getText().toString()));
                    }
                    if(chkfood.isChecked()) {
                        resources.add("Food");
                        have.add(Integer.parseInt(qtyfood.getText().toString()));
                    }
                    if(chkmedicines.isChecked()) {
                        resources.add("Medicines");
                        have.add(Integer.parseInt(qtymedicines.getText().toString()));
                    }
                    if(chktoiletries.isChecked()) {
                        resources.add("Toiletries");
                        have.add(Integer.parseInt(qtytoiletries.getText().toString()));
                    }
                    Intent intent = new Intent(getApplicationContext(), MapsActivity.class);
                    /*intent.putExtra("message", message.getText().toString());
                    intent.putStringArrayListExtra("resources",resources);
                    intent.putIntegerArrayListExtra("need",have);*/
                    ...
                    AmenityMarker amenityMarker = new AmenityMarker(latitude,longitude,resources,have,recv,message.getEditableText().toString(), date, FirebaseAuth.getInstance().getCurrentUser().getDisplayName(),phone.getEditableText().toString(), FirebaseAuth.getInstance().getCurrentUser().getUid(),true);
                    DatabaseReference ref = FirebaseDatabase.getInstance().getReference("helpers").push();
                    ref.setValue(amenityMarker);
                    String k = ref.getKey();
                    FirebaseDatabase.getInstance().getReference("helpers/"+k+"/pid").setValue(k);
